...
class CircleZohoAddonSyncService
{
    ...
    public function syncCircleAddons(Circle $circle): void
    {
        ...
        foreach ($prices as $price) {
            ...
            $intervalUnit = $duration === 12 ? 'yearly' : 'monthly';

            if ($price->zoho_addon_id) {
                $this->updateAddon($price, $name, $intervalUnit);

                continue;
            }

            $this->createAddon($circle, $price, $name, $intervalUnit, $zohoCodes);
        }
    }

    private function createAddon(Circle $circle, CircleSubscriptionPrice $price, string $name, string $intervalUnit, array &$zohoCodes): void
    {
            ...
            try {
                ...
                DB::transaction(function () use ($price, $addon, $name, $addonCode, $response): void {
                    $price->forceFill([
                        ...
                        'zoho_addon_interval_unit' => (string) (data_get($addon, 'interval_unit') ?? data_get($addon, 'interval.unit') ?? null),
                        'payload' => $response,
                    ])->save();
                });
    ...
    private function updateAddon(CircleSubscriptionPrice $price, string $name, string $intervalUnit): void
    {
        ...
        $price->forceFill([
            ...
            'zoho_addon_name' => (string) (data_get($addon, 'name') ?? $name),
            'zoho_addon_interval_unit' => (string) (data_get($addon, 'interval_unit') ?? data_get($addon, 'interval.unit') ?? $price->zoho_addon_interval_unit),
            'payload' => $response,
        ])->save();
    }
    ...
    private function reserveNextAddonCode(array $zohoCodes, ?string $ignoreId = null): string
    {
        return DB::transaction(function () use ($zohoCodes, $ignoreId): string {
            ...
            if (! $sequence) {
                DB::table('zoho_addon_code_sequences')->insert([
                    'id' => 1,
                    'next_code' => $start,
                    'created_at' => now(),
                    'updated_at' => now(),
