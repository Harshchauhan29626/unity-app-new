...
@section('title', 'Impact Pending Requests')

@section('content')
<div class="container-fluid">
    <h1 class="mb-3">Impact Pending Requests</h1>
    <div class="card">
        <div class="table-responsive">
            <table class="table table-striped mb-0">
                <thead>
                    <tr>
                        <th>Date</th><th>User</th><th>Impacted Peer</th><th>Action</th><th>Status</th><th></th>
                    </tr>
                </thead>
                <tbody>
                @forelse($impacts as $impact)
                    <tr>
                        ...
                        <td>{{ $impact->action }}</td>
                        <td><span class="badge bg-warning">{{ $impact->status }}</span></td>
                        <td><a href="{{ route('admin.impacts.show', $impact->id) }}" class="btn btn-sm btn-primary">View</a></td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="text-center py-4">No pending impact requests.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
